...
#
# This gradcam downloads PyTorch checkpoints from my Hugging Face repository for model weights.
# It is designed to work with ResNet50 and ResNet50 with CBAM (Convolutional Block Attention Module).
# The script generates Grad-CAM heatmaps for visualizing regions of interest in retinal OCT scans that
# contribute to the model's predictions. HF REPO = https://huggingface.co/jeepaz/macular-attention-oct
# =====================================================================
...
# Guarantee that local repository directories are visible to the Python path
repo_root = "/workspace/macular-attention-oct"
...
def generate_and_save_heatmap(image_path, model_name="resnet50"):
    ...
    heatmaps_path = os.path.join(repo_root, "results", "heatmaps")
    ...
    if model_name in model_mapping:
        ...
        print(f"Retrieving weights for {filename} via huggingface_hub client...")
        checkpoint_path = hf_hub_download(repo_id=REPO_ID, filename=filename)
    else:
        raise ValueError(f"Weight profile missing for model identifier: {model_name}")

    print(f"Mapping state weights from file cache: {checkpoint_path}")
    state_dict = torch.load(checkpoint_path, map_location=device)
    
    if any(k.startswith("module.") for k in state_dict.keys()):
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        print("-> Automatically stripped 'module.' prefix from checkpoint keys.")
    ...
    # RESTORED: Explicit string indexing applied securely
